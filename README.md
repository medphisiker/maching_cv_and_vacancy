 ...
 * Каждый компонент независим от других и легко может быт заменен аналогом с поддерживающим тот же унифицированный API для обмена сообщениями
 * Каждый компонент представляет собой отдельный Docker-контейнер. С одной стороны это позволит контейнеризовать рабочее окружение необходимое для работы каждого компонента. С другой даст возможность масштабирования сервиса путем увеличения числа нужных для обслуживания пользователей компонентов.