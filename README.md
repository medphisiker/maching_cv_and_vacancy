...
1. **Web-приложение.**
   ...
   Сейчас это Streamlit. CPU-сервер.
...
2. **LM-модель**
   ...
   LM-модель с Fast API для получения и POST/GET запросов и ответов. GPU-сервер.
   * создает эмбединги из присланного ей текста вакансии

3. **faiss-индекс** 
   
   faiss-index с Fast API для получения и POST/GET запросов и ответов. Возможен как CPU, так и GPU-сервер, можно выбрать на основе нагрузки/
...
4. **Базы данны** 
   ...
   сейчас это Pandas работающий с csv-файлами. Лучше заменить его на нормальную БД, например, PostgreSQL. CPU-сервер/
